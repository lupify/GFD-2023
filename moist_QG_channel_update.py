...
x = np.linspace( -Lx / 2, Lx / 2, N ,endpoint=False) #J. Kang editted
y = np.linspace( -Ly / 2, Ly / 2, N2 ,endpoint=False) #J. Kang editted
...
if init == "cold":
	lim = 50  #Start saving after this time (model time-units)
else:
	lim = 0
...
ztauP = np.zeros( ( TN , N2 ) )
zetauP = np.zeros( ( TN , N2 ) )
...
# Error in this line of code. J. Kang editted line below
            if model != "moist":
